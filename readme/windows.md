...
| Native PowerShell / CMD | Experimental, partially validated | `install.windows.ps1`, Bash-backed `pnpm run check`, Docker sandbox helper smoke, and real `/pd` Project Pipeline runs with managed preview approval have passed with Docker Desktop Linux containers. Full native Windows pipeline support is not claimed yet. |
...
- provider/delegate and broader/full pipeline scripts still need native Windows validation of delegated agent behavior, auth handling, and path quoting beyond the focused Project Pipeline smoke/preview scenarios.
...
Additional native Windows Project Pipeline evidence, refreshed on 2026-06-29 from `$HOME\pidex` with Docker Desktop Linux containers and standalone `pnpm@10.33.0`:
