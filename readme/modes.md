...
| Parallel secondary lanes | Supported by host orchestrator config with no required host-direct slowdown | Should be supported with sandbox-safe apply/extract semantics | Target support for every mode; Project Pipeline implementation is still pending |
...
| Quality reports / PDQ | Reads host PIDEX state, project artifacts, metrics, and rules; writes reports under host PIDEX/report locations | Reads host state plus sandbox evidence where available | Analyze whether existing host-state/report model already covers Project Pipeline adequately or needs archive/mode metrics; tracked as Initiative 030 F4 |
| Quality governance / rule learning | Host PIDEX quality state and rule evidence | Host PIDEX quality state plus sandbox run evidence | Host PIDEX quality state; analyze whether Project Pipeline mode/archive evidence needs additional metrics |
...
| Dashboard views | Host PIDEX dashboard reads host runtime state, metrics, wiki/context summaries, and provider data | Same host dashboard plus any sandbox events that are recorded | Needs analysis: verify whether dashboard already records/reports project mode metrics and whether mode/archive metrics should be expanded |
...
| History / metrics / recent projects | Host PIDEX metrics/history state records runs and recent projects | Host PIDEX metrics/history plus sandbox warnings/evidence when emitted | Host PIDEX history plus Project Pipeline registry/archive records; analyze whether mode labels/metrics are complete |
...
2. How to support optional parallel secondary lanes in every mode without slowing host-direct or weakening sandbox/project boundaries.
3. Whether `/pdq`, `/pdwiki`, `/pdmem`, and `/pdparallel` actually need mode-aware behavior, or whether their host-project contracts are already sufficient.
4. Whether dashboard, quality governance, and history/metrics already record enough mode evidence or need expanded mode/archive metrics.
