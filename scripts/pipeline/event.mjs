...
function parse(argv) {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    if (a === '--project') out.project = v(); else if (a === '--project-slug' || a === '--slug') out.projectSlug = v(); else if (a === '--pipeline-id') out.pipelineId = v(); else if (a === '--plan') out.plan = v(); else if (a === '--event' || a === '--event-type') out.event = v(); else if (a === '--status') out.status = v(); else if (a === '--actor') out.actor = v(); else if (a === '--message') out.message = v(); else if (a === '--source') out.source = v(); else if (a === '--metadata-json') out.metadataJson = v(); else if (a === '-h' || a === '--help') { console.log('Usage: event.mjs --plan PLAN --event EVENT [options]'); process.exit(0); } else { console.error(`Unknown arg: ${a}`); process.exit(2); }
  }
