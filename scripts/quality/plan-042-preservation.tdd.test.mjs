...
const P42 = Object.freeze({
  'config/contract-governor.json': '{\n  "$schema": "https://pidex.dev/contract-governor.schema.json",\n  "version": 2,\n  "capability": "manual-pending-only",\n  "max_proposals_per_run": 5\n}\n',
  'config/operator-contracts.schema.json': '{"type":"object"}\n',
...
function seedProtected(root) { file(root, 'config/contract-governor.json', '{"version":1}\n'); }
...
test('P42-49-V1 exact fixture preserves bytes, type, Git index, report, empty lifecycle, and real replay runtime', () => {
  ...
  try {
    assert.deepEqual(Object.keys(P42), ['config/contract-governor.json', 'config/operator-contracts.schema.json', 'state/quality/correction-ledger.jsonl', 'state/quality/report.json', 'agents/pidex-alpha.md', 'state/quality/rule-lifecycle/ledger.jsonl']);
