...
import { validateReviewOutcome, writeTbr } from './tbr.mjs';
...
function accepted(result) { return result !== false && result?.ok !== false && result?.status !== 'failed'; }
export function transitionReviewOutcome({ root, identity, outcome, write = writeTbr, appendOutcome, appendRoute, spawn }) {
  ...
  if (!accepted(appendOutcome?.(checked.value))) return { status: 'TBR_WRITE_BLOCKED', code: 'TBR_OUTCOME_APPEND_FAILED' };
  if (!accepted(appendRoute?.(checked.value))) return { status: 'TBR_WRITE_BLOCKED', code: 'TBR_ROUTE_APPEND_FAILED' };
  if (!accepted(spawn?.(checked.value))) return { status: 'TBR_WRITE_BLOCKED', code: 'TBR_SPAWN_FAILED' };
...
export function main(argv = process.argv.slice(2)) {
  try {
    ...
    if (review) { const transition = transitionReviewOutcome({ root: project, identity: { planId: record.plan_key, runFamilyId: a.runFamilyId || record.pipeline_id, reviewGate: a.gate }, outcome: obj(a.reviewOutcomeJson, 'review-outcome-json'), appendOutcome: append, appendRoute: () => true }); if (transition.status === 'TBR_WRITE_BLOCKED') throw new Error('TBR_WRITE_BLOCKED'); } else append();
