...
function latestReport(root, project) { const projectPath = path.resolve(project); return walk(path.join(resolveStateRoot({ root }), 'quality')).filter((p) => p.endsWith('.json') && path.basename(p) !== 'review-state.json').map((file) => { try { return { file, report: readJson(file) }; } catch { return null; } }).filter((x) => x?.report?.summary && path.resolve(String(x.report.project_path || '')) === projectPath).sort((a, b) => (Date.parse(b.report.generated_at || '') || 0) - (Date.parse(a.report.generated_at || '') || 0))[0]; }
function decisions(root, project) { const projectPath = path.resolve(project); return walk(path.join(resolveStateRoot({ root }), 'orchestrator-events')).filter((p) => p.endsWith('.jsonl')).flatMap(readJsonl).filter((row) => row.operator_type === 'OpDecision' && path.resolve(String(row.project_path || projectPath)) === projectPath); }
function safeRef(kind, value) { return value ? `${kind}:${hash(path.resolve(value))}` : null; }
function proposal(operatorType, reasons, evidence, contract) { const patch = { allowed_skip_reasons: reasons }; const id = `contract-correction-${hash({ operator_type: operatorType, contract_id: contract.contract_id, patch })}`; const safeDecisions = evidence.map(({ timestamp, decision_id, plan_key, operator_type, decision_type, target_operator, reason }) => ({ timestamp, decision_id: decision_id || null, plan_key: plan_key || null, operator_type, decision_type, target_operator, reason })); return { proposal_id: id, id, operator_type: operatorType, contract_id: contract.contract_id, current_contract: contract, proposed_patch: patch, reason: `Repeated explicit decisions use unsupported reasons for ${operatorType}.`, evidence: { matching_findings: [], matching_op_decisions: safeDecisions, source_reports: [], relevant_rules: [] }, impact_estimate: { affected_findings: 0, historical_reclassification: 'future-only' }, capability: 'pending-only' }; }
