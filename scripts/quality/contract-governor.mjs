...
function stateRoot(root, env = process.env) { return resolveStateRoot({ root: path.resolve(root), env }); }
function opaqueRef(kind, value) { return `${kind}:${createHash('sha256').update(path.resolve(String(value))).digest('hex').slice(0, 20)}`; }
...
export function acquireGovernorLock(root, options = {}) {
  const lockDir = path.join(stateRoot(root, options.env), 'quality/contract-governor/.lock'); const token = randomUUID(); const tokenName = `owner-${token}`; const releaseDir = `${lockDir}.release-${token}`;
...
function allDecisions(root, project) { const projectPath = path.resolve(project); return walk(path.join(stateRoot(root), 'orchestrator-events')).filter((file) => file.endsWith('.jsonl')).flatMap((file) => readJsonlStrict(file, 'GOVERNOR_EVENT_INVALID')).filter((row) => row.operator_type === 'OpDecision' && path.resolve(String(row.project_path || projectPath)) === projectPath); }
function latestReport(root, project) { const projectPath = path.resolve(project); return walk(path.join(stateRoot(root), 'quality')).filter((file) => file.endsWith('.json') && path.basename(file) !== 'review-state.json').map((file) => ({ file, report: readJsonRequired(file, 'GOVERNOR_REPORT_INVALID') })).filter((item) => item.report?.summary && path.resolve(String(item.report.project_path || '')) === projectPath).sort((a, b) => (Date.parse(b.report.generated_at || '') || 0) - (Date.parse(a.report.generated_at || '') || 0))[0] || null; }
function runId() { return `${Date.now()}-${randomUUID().slice(0, 12)}`; }
function writeRun(root, row) { const safe = { ...row, project: row.project || (row.project_path ? opaqueRef('project', row.project_path) : null), report_ref: row.report_ref || (row.report ? opaqueRef('report', row.report) : null) }; delete safe.project_path; delete safe.report; if (typeof safe.error === 'string') { safe.error = undefined; delete safe.error; } const dir = path.join(stateRoot(root), 'quality/contract-governor', safe.run_id); writeJson(path.join(dir, 'run.json'), safe); mkdirSync(path.join(root, 'agents.output/quality/contract-governor'), { recursive: true }); writeFileSync(path.join(root, 'agents.output/quality/contract-governor', `${safe.run_id}.md`), `# Contract Governor Pending-only Run\n\n- status: ${safe.status}\n- proposals: ${safe.proposals_pending || 0}\n- duplicates: ${safe.duplicates || 0}\n`, 'utf8'); return safe; }
...
function execute(args, cfg) {
  ...
  let row;
  try {
    const ledger = path.join(stateRoot(root), 'quality/contract-corrections.jsonl'); const existing = readJsonlStrict(ledger, 'GOVERNOR_LEDGER_INVALID'); let duplicates = 0;
