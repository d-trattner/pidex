...
const decisions = [1, 2].map((n) => ({ operator_type: 'OpDecision', decision_type: 'skip_step', target_operator: 'OpQualityReview', reason: 'manual-terminal-import', confidence: 'high', plan_key: `plan-00${n}`, project_path: '/private/project', raw_prompt: 'SECRET' }));
...
assert.match(first[0].evidence.source_reports[0], /^report:[a-f0-9]{16}$/); assert.doesNotMatch(JSON.stringify(first[0]), /\/tmp\/a\.json|\/private\/project|SECRET/);
