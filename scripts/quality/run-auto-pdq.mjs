...
function parse(argv) { const a = { project: ROOT, plan: 'unknown-plan', pipelineId: '', terminalEvent: 'manual' }; for (let i = 0; i < argv.length; i++) { if (argv[i] === '--manual') a.manual = true; else if (argv[i].startsWith('--')) a[argv[i].slice(2).replace(/-([a-z])/g, (_, c) => c.toUpperCase())] = argv[++i]; } return a; }
...
if (!a.manual) { console.error('MANUAL_CONFIRMATION_REQUIRED: pass --manual'); process.exit(2); }
