...
export function makeModuleFixture(options = {}) {
  ...
  writeFileSync(path.join(root, 'modules/pidex/release-safety/module.json'), JSON.stringify({
    id: 'pidex.release-safety', schema_version: 1, name: 'Release Safety', kind: 'core-toggleable', default_enabled: true, dependencies: ['pidex.core'], capabilities: [{
      id: 'release.reference-integrity', kind: 'check', phases: ['pre-release'], scope: 'install', importance: 'recommended', allowed_agents: ['orchestrator', 'pidex-devops', 'pidex-pi'], supported_platforms: ['linux', 'wsl2', 'macos', 'windows-native', 'windows-git-bash'], mutability: ['read-only'], command: { bin: 'node', args: ['scripts/release/reference-integrity.mjs'] },
    }],
  }, null, 2));
