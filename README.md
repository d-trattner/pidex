...
PIDEX is an experimental direct-mode MVP. Linux/direct mode remains the primary supported path today. Local Docker Project Pipeline is validated for the current MVP on Linux and has focused native Windows Docker Desktop evidence, including automatic managed preview approval flows for simple and dashboard-style Vite React UI fixtures.
...
PIDEX currently supports Linux/direct mode and includes experimental Windows bootstrap plus focused native Windows Docker Project Pipeline validation, including managed preview UX on Docker Desktop Linux containers.
...
- Pi CLI `>=0.78.0` installed globally:
  ...
  pi --version
  ```
- Optional Docker Project Pipeline validation: Docker Desktop running Linux containers. Native Windows Project Pipeline `/pd` smoke and dashboard-style managed preview approval runs have passed; broader native Windows pipeline support remains experimental.
