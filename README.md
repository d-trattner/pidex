...
**Unreleased development update (2026-09-13):** Astra profiles and the baseline/review/closeout/status stabilization are integrated into `master`, but have not been published as a new release. `astra-balanced` is the development default. Its bounded Linux host-direct live run completed through PI and the normal terminal acknowledgment, with one disclosed orchestrator clarification. `astra-quality` remains independently unvalidated. Package version stays `0.5.0`. See [Astra development status and validation limits](readme/astra-development-status.md) and [Unreleased changes](CHANGELOG.md#unreleased).
...
The current development focus is using the stabilized local workflow for real project work and addressing concrete recurring problems. The self-improvement loop collects quality evidence from real pipeline runs and PDQ reports to assess which rules, prompts, and workflow changes actually help. Quality Rule Learning includes operator contracts, valid operator-decision evidence, trace normalization, dashboard quality views, and a manual pending-only correction governor. The governor can record proposals but cannot run automatically, invoke a model, approve, apply, or validate contract changes. PIDEX also includes Docker-backed sandboxing in two forms: the optional hardened agent sandbox for selected source-changing pipeline work, and the validated local Project Pipeline MVP where project work runs inside a persistent Docker Project Sandbox. Both require the canonical `~/pidex` runtime checkout on Linux/WSL2 or `$HOME\pidex` for the experimental Windows bootstrap.
...
- [Astra optimization: unreleased status and validation](readme/astra-development-status.md)
...
| --- | --- | --- |
| `astra-balanced` | GPT-6 Astra for analysis, architecture, planning, critique, code review, security, roadmap and PI; other routes unchanged | Unreleased development default; bounded Linux live acceptance completed with disclosed clarification |
...
The original five-fixture benchmark selected Hybrid as the previous general default. All-role C1 screening and independent H2 holdouts then refined that GPT-5.6 profile: Hybrid Balanced uses Sol medium for Critic, Sol high for Designer and QA, and Sol high instead of xhigh for Security. Lowcost preserves the critical planning, design, implementation, and security routes but uses Terra medium for Code Reviewer and QA, where repeated holdouts showed large token reductions with correct core outcomes. Spark was tested rather than assumed unsuitable: UAT Spark high was the only eligible offload, while functional, recall, evidence, stability, latency, or operational-complexity results prevented a general Spark profile. See [Provider limits and profiles](readme/provider-limits-and-profiles.md) for the public routing summary and selection guidance.
...
Terminal pipeline lifecycle events do not dispatch PDQ automatically. Run `/pdq`, or use the compatibility runner with exact `--manual`, when a quality review is wanted. Missing manual review remains truthful trace evidence rather than fabricated completion.

See [Manual quality reports](readme/automatic-quality-reports.md) and [Quality governance](readme/quality-governance.md).
