...
This rule applies when `pidex-critic` is running as a configured parallel secondary lane, not as the primary critic lane.
...
- provider/model override indicates an auxiliary critic lane
- orchestrator asks for secondary/parallel critique

## Rule

Secondary critic lanes may write **only their assigned artifact file**.
...
If a secondary lane finds a non-blocking/deferred issue, include it in the secondary critique artifact under a clearly labelled section such as:
