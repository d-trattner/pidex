...
import { projectMirrorManifestPath, syncProjectMirror } from './project-mirror.mjs';
...
test('importLocalProject copies accepted files to container workspace and updates registry', () => {
  ...
  write(path.join(source, 'wiki/index.md'), '# Existing wiki\n');
  ...
  assert.deepEqual(result.copied.map((item) => item.path), ['README.md', 'src/app.js', 'wiki/index.md']);
  ...
  assert.equal(loaded.source.files_copied, 3);
  const mirrorManifest = JSON.parse(readFileSync(projectMirrorManifestPath(root, 'pp-import-abc123'), 'utf8'));
  assert.equal(mirrorManifest.host_root, source);
  assert.equal(typeof mirrorManifest.files['wiki/index.md'], 'string');
  assert.equal(mirrorManifest.files['README.md'], undefined);
  const updatedWiki = '# Updated in container\n';
  const archiveRoot = path.join(root, 'state/project-archives/pp-import-abc123');
  write(path.join(archiveRoot, 'wiki/index.md'), updatedWiki);
  write(path.join(archiveRoot, 'archive-sync-report.json'), JSON.stringify({ ok: true, copied: [{ path: 'wiki/index.md', size: Buffer.byteLength(updatedWiki), sha256: crypto.createHash('sha256').update(updatedWiki).digest('hex') }] }));
  const mirror = syncProjectMirror({ pidexRoot: root, projectId: 'pp-import-abc123' });
  assert.equal(mirror.status, 'complete');
  assert.equal(readFileSync(path.join(source, 'wiki/index.md'), 'utf8'), '# Updated in container\n');
});
