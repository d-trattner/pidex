...
export function traceProjectPipelineExposure({ pidexRoot, run, terminal_outcome_ref, gitTrackedPaths, env = process.env } = {}) {
  ...
  const publication = reconciliation
    ? publishCompleteBundle({ pidexRoot, identity, reconciliation, snapshot, exposure, epoch, env })
    : undefined;
  ...
  return { inventory, reconciliation, snapshot, exposure, artifacts, publication, state_root_class };
}
