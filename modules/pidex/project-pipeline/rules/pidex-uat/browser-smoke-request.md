...
UAT may emit browser-smoke request JSON under `agents.output/uat/**.json` for user-visible acceptance evidence.
...
- Use `project_id`, not `project`. The `project_id` value MUST exactly match the canonical Project Pipeline registry project_id shown in the phase prompt. Do not derive it from the app name, folder name, package name, route, or user-facing project title.
...
```json
{
  ...
  "checks": [
    { "type": "text", "contains": "<expected visible user-facing text>" },
