...
import { angular22NodeCompatibility, inspectAngularWorkspace } from './workspace-inspector.mjs';
...
test('static inspector selects Angular, Material and Nx profiles without running project commands', () => {
  const root = project({
  ...
  try {
    ...
    assert.equal(result.nx.config_present, true);
    assert.deepEqual(result.angular.projects[0].declared_targets, ['build', 'test']);
    assert.deepEqual(result.warnings, ['material_cdk_version_mismatch', 'nx_package_version_mismatch']);
    assert.equal('resolution' in result.nx, false);
