...
export function projectKeyFromResolvedPath(resolvedProject) {
  const digest = createHash('sha256').update(Buffer.from(resolvedProject, 'utf8')).digest('hex').slice(0, 12);
  return `${safeSegment(path.basename(resolvedProject))}-${digest}`;
}
...
export function canonicalProjectIdentity(project) {
  ...
  return {
    canonicalProject,
    projectKey: projectKeyFromResolvedPath(canonicalProject),
  };
}
