...
def discover_projects(args_projects: list[str]) -> list[Path]:
    ...
    # PIDEX-native default: ingest only explicitly requested PIDEX/dashboard
    # artifacts. Historical Running Pi/project artifacts (forge.ng, homelab,
    # minimax/deepseek secondary reviews, etc.) must not leak into the PIDEX
    # dashboard unless explicitly opted in for migration/debug analysis.
    include_external = parse_bool(os.environ.get("PIDEX_DASHBOARD_INCLUDE_EXTERNAL_PROJECTS"), default=False)
    if not include_external:
        if not projects:
            for p in [ROOT, ANALYTICS]:
                if p.exists() and p not in projects:
                    projects.append(p)
        return projects
