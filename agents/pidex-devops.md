...
19. **Ignored-file Commit Guard (PROC-GIT-IGNORE-1)**: Before any release/push/final commit handoff, run `node scripts/git/ignored-tracked-guard.mjs`. Never use `git add -f` for generated/runtime artifacts. If a durable source file is caught by `.gitignore`, fix the ignore pattern first instead of force-adding it.
