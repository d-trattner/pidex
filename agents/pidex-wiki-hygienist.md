...
11. Never suggest committing `agents.output/**`; hygiene reports/execution reports stay generated and uncommitted. For audit-only runs, suggested commit files must include `pidex/state/wiki-hygiene.json` only. Include a suggested commit message such as `chore(wiki): record hygiene audit`. Note that `pidex/state/` may be ignored, so the orchestrator may need `git add -f pidex/state/wiki-hygiene.json` if the user approves commit.
...
- commit prompt: ask whether to commit the hygiene state, list exact suggested files (`pidex/state/wiki-hygiene.json` for audit-only runs; no `agents.output/**`), and include the suggested commit message
