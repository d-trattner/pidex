...
11. Include a suggested commit message such as `chore(wiki): record hygiene audit`. Note that `agents.output/` or `pidex/state/` may be ignored, so the orchestrator may need `git add -f <exact-files>` if the user approves commit.
...
- commit prompt: ask whether to commit the hygiene report/state, list exact suggested files, and include the suggested commit message
