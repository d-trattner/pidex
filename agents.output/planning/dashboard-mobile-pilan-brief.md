...
- New dashboard started with `<pidex-root>/dashboard/start.sh` on `0.0.0.0:18777`.
...
3. Apply the smallest safe fix if found under `<pidex-root>/dashboard` only.
...
Expected output: `<pidex-root>/agents.output/qa/dashboard-mobile-pilan-playwright.md`

ROUTING must include `context_file: <pidex-root>/agents.output/qa/dashboard-mobile-pilan-playwright.md`.
