---
name: pidex
description: Start a pidex pipeline run. Invoke with /pidex, or by saying "pidex", "pipeline starten", "build this", "implement this". Runs a structured pre-flight interview to define the task precisely, then starts the pidex-* pipeline in Pi direct mode using the pidex_agent tool.
---

# pidex Orchestrator

This skill is the entry point for every pidex pipeline run. The user invokes it by typing `/pidex`, or saying "pidex".
...
If the fixed pre-flight interview below is not enough to remove ambiguity, load and apply the bundled `grill-me` skill (`~/pidex/skills/grill-me/SKILL.md`). Use it to interview the user relentlessly about the plan or design until all decision branches needed for the pipeline are resolved.
...
- User types `/pidex`
- User says "pidex", "pipeline starten", "start a run", "build this", "implement this"
- User describes a feature, fix, or project they want the pipeline to build
...
- `pidex_agent` honors `/home/daniel/pidex/config/agents.json`: default/tool-heavy agents use lean isolated Pi subprocesses; configured review/synthesis agents may route to Claude, Codex, or Gemini CLI delegates.
- `pidex_agent` stores raw child JSON streams under `/home/daniel/pidex/state/runs/` and returns only compact tool details to avoid parent-session bloat.
- `pidex_agent` records per-agent metrics under `/home/daniel/pidex/state/metrics/`.
- For plan-id allocation, use atomic helper `bash /home/daniel/pidex/scripts/parallel/manifest.sh next-id --project-root <project-root>` (do not increment `.next-id` ad hoc under parallel load).
...
- Optional notify-only Telegram is allowed in direct mode: use `~/pidex/scripts/telegram/notify.sh --optional` to alert the user that Pi needs attention and when the pipeline reaches terminal completion/failure/abort. This sends information only, no buttons, no replies, no `pending-gate.json`.

Before invoking the first delegated non-Pi agent, ensure delegate auth preflight is green. `/pidex` runs this automatically, but if continuing manually use:

```bash
bash /home/daniel/pidex/scripts/delegate/check-auth.sh --config /home/daniel/pidex/config/agents.json
```
...
```bash
bash ~/pidex/scripts/pre-spawn/spawn-with-budget.sh \
  --agent <pidex-agent> \
...
```bash
bash ~/pidex/scripts/history/list.sh --limit 5
```

If the script prints nothing (empty or missing `~/pidex/state/history.jsonl`), skip straight to Step 1. Otherwise present its output to the user with an explicit "new" option appended:

```
Recent projects:
A) /home/daniel/projects/local/forge.ng
   last touched: 2026-05-06T09:59:46Z (direct, direct-complete)
B) /home/daniel/projects/www/jt-sofafahrten
...
2. Use bundled agents from `/home/daniel/pidex/agents/` through the `pidex_agent` tool. Project-local `.pi/agents/` copies are optional and not required for the MVP.
...
If the user picks A-E, include in the epic: "Design template: <name> (from ~/pidex/design-templates/<name>.md — copy to agents.output/design/DESIGN.md at project init)."
...
If A-E: include in the epic "Design template: <name> (from ~/pidex/design-templates/<name>.md — copy to agents.output/design/DESIGN.md at project init)."
...
Compose the lead prompt from the template in `~/pidex/pidex-instructions.md` Rule 7 and call:

```bash
bash ~/pidex/scripts/lead/start.sh \
...
```bash
reply=$(bash ~/pidex/scripts/telegram/recv-gate.sh --wait 60)
if [ -n "$reply" ]; then
    bash ~/pidex/scripts/relay/handle.sh --text "$reply" --chat-id "$TELEGRAM_CHAT_ID"
fi
```
Loop this after each gate send until the gate clears. If `recv-gate.sh` returns empty, check back on the next monitoring tick. If the user reports "Telegram message does not arrive", relay their chosen option manually: `bash ~/pidex/scripts/relay/handle.sh --text "pidex!approve" --chat-id "$TELEGRAM_CHAT_ID"`.
...
GATE=$(jq -r '.gate' ~/pidex/state/pending-gate.json 2>/dev/null)
if [ "$GATE" = "G9" ]; then
    # Extract project cwd from active-lead.id
    LEAD_ID=$(cat ~/pidex/state/active-lead.id)
    CWD=$(cat ~/pidex/state/lead-${LEAD_ID}.cwd)
...
- **Status:** `bash ~/pidex/scripts/lead/status.sh`
- **Stop:** `bash ~/pidex/scripts/lead/stop.sh` (add `--force` if needed)
- **Replace:** `bash ~/pidex/scripts/lead/start.sh --replace --prompt "..." --cwd "..." --teams`
...
Do not copy agent files into the project. Use bundled agents from `/home/daniel/pidex/agents/` through `pidex_agent`. Then `cd` into the project (or pass the project path as `cwd` to every `pidex_agent` call).

Append a history entry so this run shows up in the next `/pidex` Step 0 shortlist, and emit an analytics-only pipeline lifecycle event for the dashboard:

```bash
bash ~/pidex/scripts/history/append.sh \
  --event direct-start \
  --cwd "<absolute project path>" \
  --mode direct \
  --epic "<confirmed epic, first 200 chars on one line>"

cd "<absolute project path>" && bash ~/pidex/scripts/pipeline/event.sh \
...
`pipeline/event.sh` is analytics-only. It writes JSONL under `~/pidex/state/pipeline-events/`; it does not drive a backend scheduler. Operators never pass SQLite `project_id`; ingest derives that from `project_path` (default `$PWD`).
...
Use the `pidex_agent` tool for every specialist handoff. Do **not** use legacy `dispatch.sh` in normal direct mode; `pidex_agent` already honors `/home/daniel/pidex/config/agents.json`, routes to Pi/Codex/Claude/Gemini as configured, stores raw child logs under `state/runs/`, and validates ROUTING/context files.
...
**Legacy plan migration:** when resuming an active plan that lacks `Execution Profile`, `Skipped Agents`, or `Retro Mode`, read `~/pidex/rules/orchestrator/legacy-plan-profile-migration.md`. Prefer a small pidex-planner revision before implementation; if late-stage, run conservative full path and brief `LEGACY-PROFILE-SKIP: no skips honored`.
...
**Parallel pidex_agent safety:** When emitting multiple `pidex_agent` calls in the same assistant turn (parallel implementer lanes, configured secondary review lanes, or post-retro handoffs), resolve `/home/daniel/pidex/config/agents.json` first and pass explicit `provider`, `model`, and `effort` for every call. Do not rely on default routing in same-turn parallel calls. Never force `provider=pi` while leaving a delegate model alias such as `sonnet`/`opus`; if overriding to Pi, use a Pi-qualified model from config defaults (for example `openai-codex/gpt-5.3-codex`).

**Orchestrator-owned parallel secondary review lanes:** For `pidex-critic`, `pidex-code-reviewer`, and `pidex-security`, inspect the agent route in `/home/daniel/pidex/config/agents.json` before spawning. If it contains `parallel_secondary`, the orchestrator owns the fan-out. `pidex_agent` must not spawn nested agents.
...
- `pidex-security`: read `~/pidex/rules/pidex-security/fallow-structural-signal.md`; record fallow evidence or `FALLOW-SKIP`.
- `pidex-qa`: read `~/pidex/rules/pidex-qa/fallow-static-audit-gate.md`; do not emit QA COMPLETE without fallow evidence or `FALLOW-SKIP`.
...
```bash
bash ~/pidex/scripts/telegram/notify.sh \
  --project "<project-cwd>" \
...
- **G9 (preview verification):** Triggered either after pidex-uat approves with `gate: G9` or after pidex-devops Stage 1 for UI-involved work before G4. The orchestrator (you) starts the dev server/preview server, determines the accessible URL, and asks the user to verify visually. Only proceed after the user says "approve". On "reject", ask what's wrong, record a scoped `MANDATORY-RETRO-TRIGGER: G9 rejection` per `rules/orchestrator/mandatory-retro-trigger-log.md`, and loop back to pidex-implementer. Skip G9 only for non-UI projects without a dev server (pure libraries, CLI tools). If pidex-uat emits `gate: G9` but the plan says G9 is not applicable or no dev server exists, treat it as routing inconsistency; do not start fake preview. Route to devops with documented correction or ask pidex-uat to resolve. Kill the dev server after G9 resolves.
...
- **G9 Preview REJECTED** → re-invoke pidex-implementer with user feedback → pidex-code-reviewer → pidex-qa → pidex-uat → pidex-devops → post-devops G9 before G4 again
