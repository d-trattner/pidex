...
node <pidex-root>/scripts/quality/preflight.mjs record \
  ...
  --context-read "<comma-separated paths read>" \
  --context-touched "<comma-separated paths touched>" \
  --acceptance-count <n> \
  --out-of-scope-count <n>
```

If exact counts are not known, use best-effort conservative values and include the context/brief artifact path in `--context-read` or `--context-touched`. Do not spawn the opening agent before this finalized `OpPreflight` is written.
...
**Review lifecycle calls:** normal same-parent reviewer/correction handoffs use minimal shape. Do not manually supply tuple fields or edit lifecycle state. Executable host boundary derives one pending identity from plan/current pointer/history; partial, ambiguous, or unmatched identity fails closed. A valid explicit full tuple remains supported; runtime boundary validates final ROUTING, local context, and durable completion before exposing route. Every correction returns `COMPLETE` to its assigned reviewer, including a bounded partial fix; the intervening reviewer records unresolved findings and may open the next correction. Never route correction-to-correction or use correction `BLOCKED` merely because findings remain. Typed completion status (`reviewCompletion`) is authoritative over contradictory ROUTING text: `CLOSED_WITH_TBR` and `USER_DECISION_REQUIRED` override any rejection route and never auto-correct; route to correction owner only on `CHANGES_REQUESTED`.

**Essential retry holds:** `PRIMARY_REVIEW_UNAVAILABLE` and essential main/adjudicator holds are typed stop results, not phase success. Stop routing, show hold to user, return control to user. Never auto-retry or promote a Secondary. Only trusted outer orchestrator may start later generation after an intervening affirmative user response, using exact single-use `resumeHoldId + resumeConfirmed:true` bound to held identity. Do not forward fields to child; reject provider/model overrides. No resume slash command exists. `DEGRADED_FAILED` Secondary is advisory only: continue Primary/other lanes; label no findings available, never approval.
...
**Project Pipeline diagnostics before agent spawning:** When troubleshooting Project Pipeline / Project Sandbox state, Docker health, dashboard visibility, archived runs, credentials status, browser-smoke infrastructure results, or native Windows no-Bash failures, use the read-only `pidex_project` tool first (`status`, `diagnose`, `runs`, `artifacts`, `credentials-status`, `preview-status`, `preview-logs`). Do not spawn `pidex-devops` or another specialist merely to inspect Project Pipeline control-plane state. If the `bash` tool fails to start `/bin/bash`, do not retry shell or ask the user to manually run `/pdproject`; use `pidex_project` and summarize the result. Spawn specialists only after the control-plane diagnosis shows source/design/QA/devops work is actually needed.
...
- **G1-G3, G5 (rejection/failure gates):** For lifecycle-tracked primary reviews, a valid in-contract rejection auto-routes bounded correction without a user gate (rejected `initial` → `correction1`; rejected `review1` → `correction2`); ordinary rejection count alone never asks user. `PRIMARY_REVIEW_UNAVAILABLE` or essential main/adjudicator hold stops routing and returns hold to user. Ask whether to retry; only later intervening affirmative response permits exact outer-orchestrator `resumeHoldId + resumeConfirmed:true` single-use call. `DEGRADED_FAILED` Secondary does not stop Primary/other lanes. Ask user directly only when typed completion status is `USER_DECISION_REQUIRED` (scope/threat/acceptance/architecture/evidence expansion), lifecycle/authority uncertain or TBR terminalization fails (`TBR_WRITE_BLOCKED`), repeated non-substantive stalls hit bounded stall breaker, or at G9/release/project boundary.
