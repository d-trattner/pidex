---
name: pd
description: Start a pidex pipeline run. Invoke only with /pd. Runs a structured pre-flight interview to define the task precisely, then starts the pidex-* pipeline in Pi direct mode using the pidex_agent tool.
---

# pidex Orchestrator

This skill is the entry point for every pidex pipeline run. The user invokes it only by typing `/pd`.
...
Before invoking the first delegated non-Pi agent, ensure delegate auth preflight is green. `/pd` runs this automatically, but if continuing manually use:
...
Append a history entry so this run shows up in the next `/pd` Step 0 shortlist, and emit an analytics-only pipeline lifecycle event for the dashboard:
