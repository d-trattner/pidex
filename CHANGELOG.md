...
- Pinned the Project Pipeline image's Pi CLI to the locally Astra-tested `0.85.1` instead of floating `latest`, with a build-time version assertion. Existing cached images require an explicit rebuild and existing project containers require separately confirmed replacement with their registered volumes preserved. Model-free image/lifecycle regression tests do not constitute Windows/Project Pipeline Astra acceptance.
...
- Added an explicit binary Angular-source restoration helper for existing CRLF drift that survives Git restore/stat-cache handling. It validates raw HEAD blobs with the unchanged integrity verifier, refuses real/staged edits, writes exact bytes and never resumes a held pipeline.
- Protected the digest-locked Angular/Material/Nx source corpus and lock mirrors against Git EOL conversion on Windows, without changing the byte/SHA integrity gate or pinned hashes. Added real Git checkout regressions and a guarded repair procedure for existing checkouts.
- Added schema-v1 Project Pipeline problem reports through the existing artifact transfer: host-validated project/run bindings, sanitized append-only project journals, event deduplication/conflict checks, and artifact-free host failure events. No Git exceptions, model calls, retry grants or automatic resume are added.
- Fixed Pi-upgrade npm config collisions by creating distinct empty user/global config files inside the isolated stage. Added a real local offline npm regression (including a shared-path negative control) and config-creation failure coverage.
- Fixed Pi-upgrade preflight rejecting the stock Node image's inherited `docker-entrypoint.sh sleep infinity` startup. Added bounded identity predicate comparisons to upgrade failures and read-only diagnostics; idle-process, ownership, mount and maintenance guards remain enforced.
- Added the separate agent-accessible `pidex_project_maintenance` tool for Pi-only upgrades with a real per-invocation user approval dialog. No self-approved flags, headless/child execution, arbitrary Docker actions or automatic continuation; `pidex_project` remains read-only.
- Added explicitly confirmed `/pdproject upgrade-pi` through the native Node/Docker bridge: pinned, staged Pi-only updates of an idle existing container, owned project execution exclusion, version checks, rollback/uncertainty dispositions and bounded maintenance diagnostics. No container replacement, Node update, credential changes, model call or automatic pipeline continuation. Model-free tests only; real Windows upgrade acceptance remains pending.
...
- Converted a real-project runaway—218 specialist dispatches, 23.86 specialist-hours, 4,870 turns, 11,371 tool calls, and approximately USD 225.76 in estimated specialist cost before manual termination—into executable lifecycle controls. Controlled test-server fixtures and a fresh anonymized real-project acceptance run confirmed pre-abort dispatches stay at zero, accepted interrupted reviews remain one-shot, independent per-gate budgets do not reset across families, ordinary non-review retry/fallback remains unchanged, and no fixture process leaked.
...
- Replaced the legacy GPT-5.4/GPT-5.5 profile catalog with GPT-5.6 Balanced, Lowcost, and Sol-quality after an initial benchmark, all-role C1 screening, and independent H2 holdouts. Spark was tested across the role matrix but not selected as a general profile: UAT was its only eligible offload, and one slower role-specific route did not justify separate profile/fallback complexity; any future Spark fallback remains exact-pair and fail closed.
