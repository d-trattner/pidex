    ...
    handlers: {
      ...
      POST: async ({ request }) => {
        ...
        if (action === 'create') {
          ...
          if (result.errors.length || !result.raw) return jsonResponse({ ok: false, errors: result.errors }, 400);
          await writeAtomic(current.safe.target, result.raw);
        } else if (action === 'approve-review-entry') {
          const result = approveReviewEntry(current.parsed.raw, Number(body.reviewIndex), body.entry);
          if (result.errors.length || !result.raw) return jsonResponse({ ok: false, errors: result.errors }, 400);
          await writeAtomic(current.safe.target, result.raw);
        } else if (action === 'delete-review-entry') {
          const result = deleteReviewEntry(current.parsed.raw, Number(body.reviewIndex));
          if (result.errors.length || !result.raw) return jsonResponse({ ok: false, errors: result.errors }, 400);
          await writeAtomic(current.safe.target, result.raw);
        } else {
