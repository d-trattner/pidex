...
if [ "$DEV" = "0" ] && [ "$BUILD" = "1" ]; then
  if [ ! -x "$VITE_BIN" ]; then
    echo "Missing dashboard dependency: vite. Run corepack pnpm install --frozen-lockfile --ignore-scripts from '$PIDEX_ROOT', or install PIDEX through the full installer." >&2
    exit 1
  fi
