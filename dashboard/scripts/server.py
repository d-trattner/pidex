    ...
    try:
        proc = subprocess.run(
            [
                ...
                "agents", "rules", "skills/runningpi", "running-pi-instructions.md",
        ...
        if status:
            markers.insert(0, {"date": "working-tree", "commit": "dirty", "label": "Uncommitted Running Pi infra changes", "kind": "proc"})
    ...
    inferred_open = rows(f"""
      WITH grouped AS (
        SELECT ar.project_id, p.name AS project, ar.plan_key,
          ...
          MAX(CASE WHEN ar.route_to IN ('rp-roadmap','user') OR ar.verdict = 'Released' OR (ar.agent IN ('rp-devops','rp-roadmap','rp-pi') AND ar.verdict IN ('COMPLETE','APPROVED')) THEN ar.timestamp END) AS completed_at,
          COUNT(*) AS agent_runs,
        ...
        try:
            ...
            if parsed.path == "/api/summary":
                ...
                pipeline = one(f"""
                  WITH started AS (
                    ...
                    FROM agent_runs ar JOIN projects p ON p.id = ar.project_id
                    WHERE ar.plan_key IS NOT NULL {where}
                      AND (ar.agent IN ('rp-devops','rp-roadmap','rp-pi') OR ar.route_to IN ('rp-roadmap','user') OR ar.verdict IN ('Released','COMPLETE'))
                  )
                ...
                return self.send_json(rows(f"""
                  WITH grouped AS (
                    SELECT
                      ...
                      ar.plan_key,
                      MIN(ar.timestamp) AS started_at,
                      MAX(CASE WHEN ar.agent IN ('rp-devops','rp-roadmap','rp-pi') OR ar.route_to IN ('rp-roadmap','user') OR ar.verdict IN ('Released','COMPLETE') THEN ar.timestamp END) AS completed_at,
                      COUNT(*) AS agent_runs,
                      COUNT(DISTINCT ar.agent) AS distinct_agents,
            ...
            if parsed.path == "/api/charts/quality":
                ...
                return self.send_json({
                    ...
                    "plannerRevisionsByPlan": rows(f"""
                      ...
                      FROM agent_runs ar JOIN projects p ON p.id = ar.project_id
                      WHERE ar.agent = 'rp-planner' {where}
            ...
            if parsed.path == "/api/charts/model-quality":
                ...
                for r in raw:
                    ...
                    is_continuation = r.get('agent') == 'rp-implementer' and v in ('BLOCKED','IN_PROGRESS')
